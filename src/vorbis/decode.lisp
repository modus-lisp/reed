...
(defvar *imdct-tables* (make-hash-table)
  "N -> the cosine matrix for a block of that size, built once and shared.")

(defun %imdct-table (n)
  (or (gethash n *imdct-tables*)
      (setf (gethash n *imdct-tables*)
...
(defun %imdct (spectrum n out)
  ...
  (let ((tab (the (simple-array double-float (*)) (%imdct-table n)))
        (half (ash n -1)))
